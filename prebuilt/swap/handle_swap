...
#-------------------------------------------
#
# This script will create a swap image on
# "/dataint" which needs to be mounted to
# internal NAND storage of HTC Desire.
#
# Matthias (spezi77) - (20160221)
#
#-------------------------------------------
...
# partition /cache or /dataint or /system
partition="/dataint";

# the requested swap file size in MB
swap_file_size_mb=400;
...
if [ $partition = "/cache" -o $partition = "/dataint" -o $partition = "/system" ] ;
then
    ...
    echo "\t/cache or /dataint or /system" >> $log_file;
...
then
    ...
    df "/system" "/dataint" "/cache" >> $log_file;
